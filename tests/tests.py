...
import time
import random
from netsim.simulation import Link
...
def verify_routing_table(net, source, entries, verbose=True):
    ...
    for dest, expected_route in entries.items():
        ...
        if route in expected_route:
            continue
        if verbose:
            exp = (' or '.join(expected_route)
                   if expected_route[0] is not None else None)
            print('error in routing table: node %s, dest %s, expected %s, got %s'
                  % (source, dest, exp, route))
        return False
    return True
...
def verify_routes(network):
    ...
    result = verify_routing_table(net_euclidean, 'A', {
        'B': ('B',),
        'C': ('B',),
        'D': ('B',),
        'E': ('E',),
    ...
    result &= verify_routing_table(net_euclidean, 'B', {
        'A': ('A',),
        'C': ('F',),
        'D': ('F',),
        'E': ('A',),
    ...
    result &= verify_routing_table(net_euclidean, 'C', {
        'A': ('F',),
        'B': ('F',),
        'D': ('D',),
        'E': ('F',),
    ...
    result &= verify_routing_table(net_euclidean, 'D', {
        'A': ('C', 'G'),
        'B': ('C', 'G'),
        'C': ('C',),
        'E': ('C', 'G'),
    ...
    result &= verify_routing_table(net_euclidean, 'E', {
        'A': ('A',),
        'B': ('A',),
        'C': ('A',),
        'D': ('A',),
    ...
    result &= verify_routing_table(net_euclidean, 'F', {
        'A': ('B',),
        'B': ('B',),
        'C': ('C',),
        'D': ('C', 'G'),
    ...
    result &= verify_routing_table(net_euclidean, 'G', {
        'A': ('F',),
        'B': ('F',),
        'C': ('C',),
        'D': ('D',),
    ...
    result &= verify_routing_table(net_euclidean, 'H', {
        'A': ('G',),
        'B': ('G',),
        'C': ('D', 'G'),
        'D': ('D',),
    ...
    net.step(count=1000)
    # specify dest:first-hop to be checked
    result = verify_routing_table(net, 'A', {
        'B': ('C',),
        'C': ('C',),
        'D': ('C',),
        'E': ('C',),
    ...
    result &= verify_routing_table(net, 'B', {
        'A': ('C',),
        'C': ('C',),
        'D': ('C',),
        'E': ('E',),
    ...
    result &= verify_routing_table(net, 'C', {
        'A': ('A',),
        'B': ('B',),
        'D': ('D',),
        'E': ('B',),
    ...
    result &= verify_routing_table(net, 'D', {
        'A': ('C',),
        'B': ('C',),
        'C': ('C',),
        'E': ('C',),
    ...
    result &= verify_routing_table(net, 'E', {
        'A': ('B',),
        'B': ('B',),
        'C': ('B',),
        'D': ('B',),
    ...
    result &= verify_routing_table(net, 'F', {
        'A': ('D',),
        'B': ('D',),
        'C': ('D',),
        'D': ('D',),
    ...
    LINKS = (('A', 'B'), ('A', 'E'), ('B', 'F'),
             ('B', 'C'), ('C', 'F'), ('C', 'D'),
             ('D', 'G'), ('D', 'H'), ('E', 'F'),
             # this last link is going to start out broken
             ('F', 'G'), ('G', 'H'), ('C', 'G'))
    ...
    result &= verify_routing_table(net, 'G', {
        'A': ('F',),
        'B': ('F',),
        'C': ('D', 'F', 'H'),
        'D': ('D', 'H'),
    ...
    result &= verify_routing_table(net, 'H', {
        'A': ('G',),
        'B': ('G',),
        'C': ('D',),
        'D': ('D',),
...
def convergence_time(net, correct_routing_table):
    ...
    for i in range(100000):
        ...
        for src in correct_routing_table.keys():
            result &= verify_routing_table(
                net, src, correct_routing_table[src], verbose=False)
        if result:
            correct_count += 1
        else:
            correct_count = 0
...
# To run these tests, call verify_routes with the desired network class.
# For example:
# verify_routes(RouterNetwork)
